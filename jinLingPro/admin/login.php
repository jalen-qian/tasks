...
if(isset($_POST['username'])){
	...
	//$rowCount = $myPdo -> isOne(['user'],["name = '{$_POST['username']}'","password = '".md5($_POST['password'])."'"]);
	$result = $myPdo -> select(['user'],['id'],['where'=> ["name = '{$_POST['username']}'","password = '".md5($_POST['password'])."'"]]);
	if(isset($result) && isset($result[0]) &&  $result[0]->id > 0){
		$userId = $result[0] -> id;
		$mSession->set('username',$_POST["username"]);
		$mSession->set('userId',$userId);
		$mSession->set('isLogin',true);
		echo '<script>';
		if(strlen(SID) > 0){
		...
		echo "<script>alter(\"用户名或者密码错误！\")</script>";
	}
}
?>
            ...
            <tbody>
                <tr>
                    ...
                    <table class="login_tab">
                    	<tr>
                        	<td>用户名:</td>
                            <td><input type="text" value="" id="username" tabindex="1" class="txt" name="username"/></td>
                        </tr>
                        <tr>
                        	<td>密　码:</td>
